...
export default function PacksPage() {
  ...
  if (isLoading) {
    return (
      <div className="space-y-4">
        <h1 className="text-2xl font-bold text-gray-900">Listing Packs</h1>
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Listing Packs</h1>
        <p className="text-sm text-gray-500">{data?.total ?? 0} packs total</p>
      </div>
                ...
                <tbody className="divide-y divide-gray-100">
                  {data?.items.map((pack) => (
                    <tr key={pack.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-gray-700">{pack.listing_count}</td>
                      <td className="px-4 py-3 text-gray-700">${pack.price.toLocaleString()}</td>
